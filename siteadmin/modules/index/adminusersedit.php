    ...
    if ($conn->Affected_Rows() != 1) {
        ...
        if ( isset($_POST['edit_admin_user']) ) {
            ...
            if ($admin['role'] == 'admin') {
                ...
                $sql = "SELECT role, is_active FROM admin_users WHERE id = " .$AID. " LIMIT 1";
                $rs = $conn->execute($sql);
                $current_role = $rs->fields['role'];
                $current_active = intval($rs->fields['is_active']);

                if ($current_role == 'superadmin' && $current_active == 1 && $total_superadmins <= 1) {
            ...
            if ($admin['is_active'] == '0') {
                $sql = "SELECT role FROM admin_users WHERE id = " .$AID. " LIMIT 1";
                $rs = $conn->execute($sql);
                if ($conn->Affected_Rows() == 1 && $rs->fields['role'] == 'superadmin') {
                    $sql = "SELECT COUNT(id) AS total_superadmins FROM admin_users WHERE role = 'superadmin' AND is_active = 1";
                    $rs = $conn->execute($sql);
